 ...
 *
 * Usage:
 *   npm run deploy                  # testnet (default, contest)
 *   npm run deploy -- --regtest     # regtest
...
const isRegtest = process.argv.includes('--regtest');
// Default = testnet (contest network)
...
const NETWORK_LABEL = isMainnet ? 'mainnet' : isRegtest ? 'regtest' : 'testnet';
const CURRENCY = isMainnet ? 'BTC' : 'rBTC';
const FAUCET_URL = 'https://faucet.opnet.org';

const POLL_INTERVAL_MS = 10_000;
const MAX_POLL_MS = isMainnet ? 30 * 60_000 : isRegtest ? 15 * 60_000 : 10 * 60_000;
...
async function main(): Promise<void> {
    ...
    // Read current contracts.ts and update only the current network's address
    let contractsSrc = fs.readFileSync(CONTRACTS_TS, 'utf-8');
    const addrRegex = new RegExp(`(${NETWORK_LABEL}:\\s*')[^']*(')`);
    if (addrRegex.test(contractsSrc)) {
        contractsSrc = contractsSrc.replace(addrRegex, `$1${contractAddress}$2`);
    } else {
        // Fallback: insert before closing brace of ADDRESSES
        contractsSrc = contractsSrc.replace(
            /};/,
            `    ${NETWORK_LABEL}: '${contractAddress}',\n};`,
        );
    }
    fs.writeFileSync(CONTRACTS_TS, contractsSrc, 'utf-8');
    console.log(`  Updated: frontend/src/config/contracts.ts [${NETWORK_LABEL}]`);
