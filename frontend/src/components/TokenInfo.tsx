...
export function TokenInfo({ address }: TokenInfoProps): React.JSX.Element | null {
    ...
    if (loading) {
        return <div className="token-info loading">Loading token info...</div>;
    }

    if (error) {
        return <div className="token-info error">Error: {error}</div>;
    }
